 ...
 */
function buildContextAndSources(chunks: any[]) {
  const context = chunks
    .map(
      (chunk, i) =>
        `Source ${i + 1} — "${chunk.title}" (relevance: ${(chunk.similarity * 100).toFixed(1)}%):\n${chunk.chunkText}`
    )
    .join("\n\n---\n\n");
...
export const chatWithBrain = async (req: Request, res: Response) => {
  try {
    ...
    const answer = await generateAnswer(context, query.trim());
