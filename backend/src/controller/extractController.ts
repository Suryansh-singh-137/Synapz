  ...
  try {
    ...
    if (content.status === "extracted") {
      return res.status(200).json({
        ...
        extractedText: content.extractedText?.substring(0, 100) + "...",
      });
    }

    // Step 3: Extract text from URL
    console.log(`Extracting text from: ${content.link}`);
    const extractedText = await extractTextFromUrl(content.link);
 ...
 */
export const extractAllContent = async (req: Request, res: Response) => {
  try {
    const userId = (req as any).userId;
